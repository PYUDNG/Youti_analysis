...
Class YoutiAPI
	Private Sub Class_Initialize()
		...
		If Read(CertificateFilePath) <> 0 Then Call Youti_Auto_Login(): Call Save(CertificateFilePath)
	End Sub
	...
	Public Function Youti_GetHomeworkList(ByVal PaperType, ByVal Status)
		...
		Case Else ' Token失效（如：600代表账号在另一个地方登陆），再次获取Token并保存
		    ...
		    Call Youti_Auto_Login()
		    Set Youti_GetHomeworkList = Youti_GetHomeworkList(PaperType, Status)
		    Call Save(CertificateFilePath)
		End Select
	End Function
	
	Private Function Youti_ForceLogin()
		''' 忽略已有账号信息、sessionid和authoriztion，要求用户重新登录；当用户拒绝登录时，返回False '''
		
		CL.LogInfo "Call Youti_ForceLogin"
		
		' 检查程序模块是否完整
		If Not FSO.FileExists(SelfFolderPath & "账号密码加密储存.vbs") Then 
			MsgBox "丢失 [账号密码加密储存.vbs]！请重新下载此文件并放在本程序同一目录下！" & vbCrLf & "程序正在退出..."
			Youti_Auto_Login = -1
			CL.LogError "Model Missing: [账号密码加密储存.vbs]"
			WScript.Quit
		End If
		
		' 清除账号信息
		If FSO.FileExists(AccountFilePath)     Then FSO.DeleteFile AccountFilePath,     True
		' 清除认证信息
		If FSO.FileExists(CertificateFilePath) Then FSO.DeleteFile CertificateFilePath, True
		
		' 用户设置账号信息并自动保存
		ws.Run "wscript.exe """ & SelfFolderPath & "账号密码加密储存.vbs" & """", 1, True
		
		' 用户拒绝登录时，返回False
		If Not FSO.FileExists(AccountFilePath) Then Youti_ForceLogin = False: Exit Function Else Youti_ForceLogin = True
		
		' 使用保存的账号信息登录
		Call Youti_Auto_Login()
	End Function
	
	Private Function Youti_Auto_Login()
		...
		If VarType(AccountInformation) = 2 Then
			...
			If Not Youti_ForceLogin() Then WScript.Quit
			AccountInformation = OAS.ReadAccount(AccountFilePath)
		End If
	...
	Private Function Youti_Login(ByVal Account, ByVal Phone, ByVal Password)
	    ...
	    ' 获取消息体和cookies
	    Set Body = VJ.Decode(HTTP.responseText)
	    ...
	    If Body("code") <> 200 Then
	    	Dim ErrorMsg
	    	ErrorMsg = _
	    		"登录失败！" & vbCrLf & vbCrLf & _
	    		"优题网返回的错误信息：" & Body("errors")
	    	If Body("msg") <> "" Then ErrorMsg = ErrorMsg & vbCrLf & "优题网返回的消息：" & Body("msg")
	    	ErrorMsg = ErrorMsg & vbCrLf & vbCrLf & "综上，请重新登录。。"
	    	MsgBox ErrorMsg, 16 + 4096, "T_T"
	    	If Not Youti_ForceLogin() Then WScript.Quit
	    	Exit Function
	    End If
...
'		authorization = re.Replace(authorization, "")
	...
	Function MessageBox(ByVal Content, ByVal Icon, ByVal Title, ByVal DisplayTime)
		''' 可以一次弹出多个、设定超时时间，且拥有返回值的信息框函数；返回一个WshShell的Exec对象，使用该对象的StdOut.ReadLine方法可以获取返回值；DisplayTime为显示超时时间（单位：毫秒），超过此时间信息框自动关闭，如果此数值小于0则不自动关闭；（重要）自动关闭的信息框不提供返回值，只有不自动关闭的信息框提供返回值 '''
		...
		MSGFileContent = "'On Error Resume Next" & vbCrLf &_
						 "Dim R" & vbCrLf &_
						 "CreateObject(""Scripting.FileSystemObject"").DeleteFile Wscript.ScriptFullName" & vbCrLf &_
						 "R = Msgbox(""" & Content & """," & CStr(Icon) & ",""" & Title & """)" & vbCrLf &_
						 "If LCase(Right(WScript.FullName, 12)) = ""\cscript.exe"" Then WScript.StdOut.WriteLine CStr(R)"
		If DisplayTime < 0 Then
			ExecFileContent = "'On Error Resume Next" & vbCrLf &_
							  "CreateObject(""Scripting.FileSystemObject"").DeleteFile Wscript.ScriptFullName" & vbCrLf &_
							  "Dim MsgProcess, R" & vbCrLf &_
							  "Set MsgProcess = CreateObject(""Wscript.Shell"").Exec(""cscript.exe //NOLOGO //e:VBScript "" & Chr(34) & """ & MsgFilePath & """ & Chr(34))" & vbCrLf &_
							  "R = MsgProcess.StdOut.ReadLine" & vbCrLf &_
							  "WScript.StdOut.WriteLine CStr(R)"
		Else
			ExecFileContent = "'On Error Resume Next" & vbCrLf &_
							  "CreateObject(""Scripting.FileSystemObject"").DeleteFile Wscript.ScriptFullName" & vbCrLf &_
							  "Dim MsgProcess" & vbCrLf &_
							  "Set MsgProcess = CreateObject(""Wscript.Shell"").Exec(""wscript.exe //NOLOGO //e:VBScript "" & Chr(34) & """ & MsgFilePath & """ & Chr(34))" & vbCrLf &_
							  "Wscript.Sleep " & Cstr(DisplayTime) & vbCrLf &_
		...
		If DisplayTime < 0 Then
			Set MessageBox = ws.Exec("cscript.exe //NOLOGO //e:VBScript """ & ExecFilePath & """")
		Else
			ws.Run "wscript.exe //e:VBScript """ & ExecFilePath & """", 0
